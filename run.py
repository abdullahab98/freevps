...
Notes:
  - "max" timeout means no explicit timeout_seconds is sent to HopX.
    HopX documents sandbox creation without a timeout for development use.
...
try:
    ...
    HopxError = AuthenticationError = CodeExecutionError = RateLimitError = Exception
...
# HopX supports creating a sandbox without an explicit timeout.
# "max" therefore means: do not send timeout_seconds.
...
def parse_timeout_choice(raw: str) -> Optional[int]:
    raw = raw.strip().lower()
    if raw in {"", "max", "m", "maximum", "none", "0", "no"}:
        return None

    try:
        seconds = int(raw)
    except ValueError:
        raise ValueError("Timeout must be an integer number of seconds, or 'max'")

    if seconds < 1:
        raise ValueError("Timeout must be >= 1 second, or use 'max' for no explicit timeout")
...
def create(api_key: str) -> Any:
    ...
    timeout = parse_timeout_choice(timeout_raw)
    ...
    info(
        ...
        + (f", timeout={timeout}s ..." if timeout is not None else ", no explicit timeout ...")
    )

    try:
        sb = Sandbox.create(**kwargs)
    except Exception as e:
        raise RuntimeError(f"Could not create sandbox: {e}") from e

    set_current(sid_of(sb))
    ok(f"created {sid_of(sb)}")
    ok("timeout: no explicit timeout" if timeout is None else f"timeout: {timeout}s")
    show_info(sb)
    return sb
    ...
    while True:
        ...
        try:
            ...
            if cmd == "py":
                ...
                if success is False and not err:
                    error = getattr(res, "error", "Python execution failed")
                    print(color(str(error), C.red))
                continue
...
def menu(api_key: str) -> None:
    while True:
        ...
        try:
            ...
            bad(f"Authentication error: {e}")
            pause()
        except RateLimitError as e:
            bad(f"Rate limit: {e}")
            pause()
        except CodeExecutionError as e:
            bad(f"Code execution error: {e}")
            pause()
        except HopxError as e:
            bad(f"HopX API error: {e}")
            pause()
        except Exception as e:
            bad(str(e))
            pause()
